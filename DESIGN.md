...
- **Cross-channel continuity:** Sharing is allowed only when the channels resolve to the same verified `AudienceIdentity` and that contact has explicitly consented to cross-channel memory. Anonymous or unverified identities remain channel-isolated, and revoking consent blocks shared recall immediately.
