...
- **Approval budgets:** Present pre-execution governance limits as estimated Tokens, never currency. Keep these visually and verbally distinct from service credits, wallet balances, and actual provider-cost accounting.
...
- Dashboard v2 top-level information architecture is fixed to: Overview, Knowledge Library, Digital Representatives, Inbox, Approvals, Skills, Wallet, Analytics, Channels, Audit Logs, and Settings. Memory is configured only inside `Digital Representatives -> Configuration -> Memory`; it is not a workspace-level destination.
...
- **Workspace flow:** Keep three persistent module-level destinations: representative directory and creation, five-step configuration, and publish/operate. Creation opens the new representative's configuration immediately; one Owner may create multiple independently configured representatives.
...
- **Message authorship:** Audience, digital representative, operator, system, and tool messages must be visually and textually distinct. Human operator messages never masquerade as the representative. Visitor attachments remain attached to their source message and are available to the active Operator through an Owner-scoped preview/download route; raw object keys never enter the UI.
...
## Representative Memory Configuration Pattern
- **Placement:** Memory has no workspace-level navigation item or standalone owner console. Its only Owner entry is `Digital Representatives -> Configuration -> Memory`, scoped to the selected representative.
- **Scope:** The feature controls Contact Memory, Representative Experience, same-conversation Short-term Memory, channel capability, retention, and OpenViking projection health. It is not another public-knowledge editor, training console, approval queue, or raw OpenViking browser.
- **Knowledge boundary:** Public FAQs, materials, policies, files, URLs, drafts, bindings, and published versions belong exclusively to Knowledge Library. Memory settings must not duplicate knowledge creation, review, publishing, projection, or deletion workflows.
- **Primary hierarchy:** Use four compact settings groups: Basic policy, Channel capability, Retention, and Advanced diagnostics. Lead with the behavioral effect and safety boundary; keep server-managed namespace, target URI, provider state, and reconciliation coverage read-only.
- **Automated policy:** Memory extraction never waits for an Owner or Reviewer. The system creates an internal candidate, runs deterministic source, scope, safety, minimization, and confidence checks, and automatically chooses activate, update, no-change, block, quarantine, delete, or retry. Uncertain, malformed, sensitive, or unsupported inputs fail closed and remain non-recallable.
- **Internal governance:** Candidate isolation, immutable versions, automated-policy audit, usage ledger, deletion proof, retry state, and reconciliation remain server-side controls even though no candidate list or memory-approval UI is exposed.
- **Contact Memory:** Contact facts do not require de-identification because they are used only for that contact. They must still be structured, minimized, category-limited, and scoped to one representative and contact. Names, handles, email addresses, phone numbers, account IDs, and other identity fields remain in Contact/Profile records and must not be copied into semantic memory text.
- **Cross-channel continuity:** Sharing is allowed only when the channels resolve to the same verified `AudienceIdentity`. For a verified signed-in contact the user-owned preference defaults on after the current disclosure is recorded; only that contact can turn it off. Anonymous or unverified identities remain channel-isolated, Owner configuration cannot override the contact preference, and revoking consent blocks shared recall immediately.
- **Representative Experience:** Experience captures de-identified, non-personal patterns aggregated from multiple eligible contacts or conversations. A single message or contact may not activate representative-wide experience. It must never contain contact-specific facts, payment truth, credentials, private notes, raw conversation transcripts, or reversible identifiers, and it never crosses representative boundaries.
- **Short-term memory:** Short-term context is limited to the current conversation or episode, uses bounded recent turns, and is discarded with that context. It is never projected to OpenViking and never becomes long-term memory merely because a handoff or conversation completes.
- **Same-conversation recall:** When a visitor explicitly asks what they just said, return the latest eligible visitor-authored message deterministically and label it as based on the current conversation record. Do not present this as long-term memory, governed recall, or model inference; do not charge for it.
- **Recall trace:** The product may show operational counts only where they are actionable, distinguishing searched, scope/safety allowed, injected, and publicly cited. Internal URIs, model scores, prompts, session IDs, and other contacts' information stay private; only context actually injected into a successful reply counts as used.
- **Withdrawal and deletion:** Policy changes, contact requests, consent withdrawal, source redaction, expiry, and deletion block new recall immediately. Under healthy service conditions, physical cleanup converges locally and remotely within 60 seconds; remote failure keeps the record blocked and produces a retryable, content-free deletion proof.
- **Sync truth:** PostgreSQL is authoritative and OpenViking is a rebuildable search projection. The settings surface states the sync strategy and truthfully distinguishes healthy, queued, running, partial coverage, retrying, and failed states; retries are idempotent and newer active versions fence older work.
- **Permissions:** Owners configure representative policy; contacts control consent and their own memory through supported contact-facing flows; Operators receive only the minimum context needed for the active service. Removal of memory approval must not change the separate, generic Approvals product or its authorization model.
- **Public disclosure:** Before first use, each supported channel explains what can be remembered, what is excluded, the representative/contact scope, retention, cross-channel behavior, and deletion path. Unsupported channels state that memory recall/extraction is unavailable; internal implementation vocabulary stays out of public surfaces.
- **Status language:** Color supports but never replaces a textual state, reason, and recovery action. Teal indicates active/trusted, indigo indicates automated processing or synchronization, amber indicates retryable attention, red indicates blocked failure, and neutral gray indicates disabled, expired, or deleted history.
- **Responsive behavior:** Configuration groups become one column on narrow screens. Descriptions, channel support text, retention consequences, and read-only diagnostics remain visible; no dense candidate table or review action rail is introduced.
...
## Owner Settings Pattern
- **Scope:** Settings belongs to the authenticated Owner, not the active representative. Preserve representative context only so leaving Settings can return to it; never use `rep` to authorize, read, or write Owner settings.
- **Navigation:** Keep two direct-link destinations: Profile & preferences and Notifications. Group avatar, nickname, and sign-in methods in one Account information card; place Sign out in a separate card at the bottom of Profile & preferences. Legacy Sign-in & security links resolve to Profile & preferences. Do not add Workspace members, Billing, Developer, or integration placeholders until those domains have real product contracts.
- **Saving:** Profile/preferences and notification preferences save independently. Every form has explicit Save and Discard actions, visible pristine/dirty/saving/success/error states, and optimistic-conflict recovery. There is no page-wide decorative Save button.
- **Identity boundary:** Delegate may edit the private Owner account display name, time zone, and interface language. The account display name must not overwrite the legacy/public Owner attribution used by representative pages. Logto email, phone, and verification state are read-only and must reflect channel-specific verification evidence. Never infer email verification from a verified phone or a provider-level timestamp.
- **Security truth:** Show only the connected identity provider and current browser session behavior that the product can prove. Account management links appear only when a validated account-center URL is configured. Do not invent MFA, device history, security scores, or session revocation controls.
- **Notifications:** Until an external delivery worker exists, notification preferences control real Dashboard navigation reminders only. Handoff, approval, and channel-health reminders are optional; unresolved wallet exceptions are mandatory. Never label email, SMS, Slack, or webhook delivery as active without a complete delivery path.
- **Audit and concurrency:** Owner setting writes are owner-scoped, private, non-cacheable, optimistic, idempotent, and audited in the same transaction. Audit payloads contain changed-field names and safe operational values, never email, phone, identity metadata, or display-name history.
- **Empty and unavailable states:** A database-free development state is explicitly unavailable and read-only. Do not populate fake members, rule counts, integration counts, health scores, or successful save feedback.
- **Responsive behavior:** Section links may scroll horizontally; form fields collapse to one column; actions become full-width where needed; every interactive target is at least `44px` on narrow screens.
- **Accessibility:** Section navigation uses links with `aria-current`, not fake tabs. Form errors are associated with fields, async feedback uses status/alert live regions, and every reminder state is expressed in text as well as color.
...
- **Durability:** Restore the server timeline on load. Submission shows an accepted/pending state, final content arrives asynchronously, and refreshing the page never discards persisted messages. Governed multi-step tasks and inline TurnPlan execution show their current server-persisted status and ordered step states; the UI must not infer progress from terminal chat messages. Public TurnPlan progress may reveal sanitized goals, deliverables, the current phase, bounded document-part progress, and elapsed time, but never hidden reasoning, raw prompts, private context, credentials, tool arguments, or internal policy traces.
- **Authorship and handoff:** Digital representative and human Operator messages are visually and textually explicit. While a request is waiting, the visitor can cancel it; when human control is active, keep the composer available for additional context and provide a confirmed “End human service and return to AI” action. A consumed handoff entitlement is not presented as refundable.
...
| 2026-07-17 | Restored the Owner 1:N representative workspace flow | Makes creation, five-step draft configuration, publish readiness, immutable public runtime, and version reactivation reachable in one module without changing Owner identity from representative settings. |
...
| 2026-08-06 | Moved Memory into representative configuration and automated policy | Removes the standalone Memory System and human memory-approval workflow while retaining fail-closed candidate isolation, versioning, audit, deletion, usage, and reconciliation controls; keeps Knowledge Library and generic Approvals as separate products. |
