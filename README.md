...
| Agent consumes service credits | Implemented for public conversation flow | Acceptance atomically chooses a conversation-locked free slot or reserves the same canonical audience units in both the wallet and entitlement ledgers before publishing work. Successful replies and approved non-MCP Compute settle both; non-billable, rejected, edited, redacted, canceled, and terminal-failure paths transfer or release both, including retry-backoff runs. MCP-only answers require no Pass, release any provisional reservation after planning, and consume neither purchased units nor free-reply usage; mixed MCP + non-MCP tasks retain the non-MCP billing contract. Multi-step billable tasks transfer one server-verified run owner and finalize it exactly once; generation writes are fenced by renewable worker leases. Wallet/entitlement drift fails closed on a consistent snapshot, and the package root exposes only the composite dual-ledger usage lifecycle. |
...
- `DELEGATE_CREATOR_ADMISSION_MODE` is `invite_only` by default and may be explicitly changed to `self_service`. In self-service mode, only a short-lived signed `flow=register` authorization may create a new Creator; ordinary sign-in never upgrades an Audience account. `DELEGATE_CREATOR_ADMISSION_PRINCIPALS` remains the exact `issuer|subject` allowlist used by invitation mode. Existing Owner identity links continue to work after removal; subject-only entries and wildcards are rejected.
- `DELEGATE_ACCOUNT_SESSION_MODE` is the finite `legacy | shadow | enforce | contract` rollout state for Account/AppSession v2 and defaults to `legacy`. `legacy` never accesses the v2 account/session tables. `shadow` keeps signed legacy cookies authoritative while dual-issuing opaque v2 cookies. `enforce` and `contract` read only the database-backed `delegate_dashboard_session_v2` or `delegate_reps_session_v2` cookie, revalidate Account/AuthIdentity/persona state, and never fall back to legacy authentication. Public AppSessions persist the proof-bound browser audience ID so their identity remains stable across representative pages. Logout revokes the opaque token in every v2-enabled mode.
- `LOGTO_WEBHOOK_SIGNING_KEY` verifies the raw-body HMAC from Logto's `logto-signature-sha-256` header at `/api/auth/logto/webhook`. Subscribe that endpoint to `User.SuspensionStatus.Updated` and `User.Deleted`; suspension is reversible, deletion marks the Delegate Account `DELETION_PENDING`, and both revoke local AppSessions.
- `LOGTO_MANAGEMENT_APP_ID` / `LOGTO_MANAGEMENT_APP_SECRET` belong only to the workflow runner. The previous local `LOGTO_M2M_APP_ID` / `LOGTO_M2M_APP_SECRET` names remain accepted as a migration alias. When configured, the runner obtains a client-credentials token for `LOGTO_MANAGEMENT_API_RESOURCE`, lists every Logto user with bounded pagination, and reconciles missed suspension, reactivation, and deletion events every `LOGTO_RECONCILIATION_POLL_MS`. A partial listing or page-cap hit fails closed and never infers deletion.
- In `shadow` mode, Dashboard and Reps compare the legacy principal with the opaque v2 principal. Every mismatch and a deterministic 1% match sample are emitted as `account_session_shadow_parity` structured events without token, issuer, subject, email, or persona IDs. Do not switch to `enforce` until mismatch events remain at zero for the observation window.
