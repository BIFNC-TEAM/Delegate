...
| Agent consumes service credits | Implemented for public conversation flow | Acceptance atomically chooses a free slot or reserves scoped credits. Successful replies and approved Compute settle them; non-billable, rejected, canceled, and terminal-failure paths release them. Multi-step Compute/MCP tasks transfer one server-verified billing context between runs and finalize it exactly once; generation writes are fenced by renewable worker leases. Provider/platform cost hooks are recorded by settlement. Other runtime lanes can adopt the same reserve/settle API incrementally. |
