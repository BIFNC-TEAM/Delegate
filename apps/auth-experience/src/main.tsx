...
function App() {
  ...
  const submitLogin = (event: React.FormEvent) => {
    ...
    setBootError("");
    invoke(() => mode === "sms" ? flow.codeLogin(account, code) : flow.passwordLogin(account, password));
  };
  ...
  return <main><section className="auth-card"><a className="brand" href="#" onClick={(e) => e.preventDefault()}><span>D</span>Delegate</a><p className="eyebrow">你的数字代表，从这里开始</p>
    ...
    {(state.stage === "login" || state.stage === "link") && <>
      ...
      <form onSubmit={submitLogin}><label><span>{mode === "sms" ? "手机号" : "账号"}</span><div className="phone-row">{mode === "sms" && <span className="country">+86</span>}<input value={account} onChange={(e) => setAccount(e.target.value)} autoComplete="username" inputMode={mode === "sms" ? "tel" : "text"} placeholder={mode === "sms" ? "请输入手机号" : (settings?.signIn?.methods?.some((method) => method.identifier === "email") ? "手机号、邮箱或已有用户名" : "手机号或已有用户名")} /></div></label>
    ...
    {state.stage === "reset" && <><h1>重置登录密码</h1><p className="muted">使用已绑定的中国大陆手机号验证身份。</p><label><span>已绑定手机号</span><input value={account} onChange={(e) => setAccount(e.target.value)} autoComplete="username" placeholder="请输入中国大陆手机号" /></label>{codeField}<button className="full" disabled={busy} onClick={() => invoke(() => flow.codeLogin(account, code))}>验证并继续</button><button className="text-button back" disabled={busy} onClick={() => invoke(() => flow.reset())}>返回登录</button></>}
