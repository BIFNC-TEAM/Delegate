...
import { resolveProfileBindingChannels } from "../app/reps/[slug]/profile-binding-presentation";
...
describe("public representative profile inspector", () => {
  ...
  it("uses real server-backed channel state without duplicating the services workspace", () => {
    ...
    expect(inspectorSource).not.toContain("representative-profile-recent-order");
    expect(inspectorSource).not.toContain("representative-profile-package-preview");
  ...
  it("counts account-level Telegram and Matrix bindings independently of runtime endpoint readiness", () => {
    ...
    })).toEqual({ telegram: "linked", matrix: "linked" });
  });
  ...
  it("uses progressive disclosure for secondary profile information", () => {
    ...
    expect(inspectorSource).toContain("representative-inspector-profile-card");
    expect(inspectorSource).toContain("representative-inspector-details-card");
    expect(inspectorSource).toContain("representativeInfoLabel");
    expect(inspectorSource).toContain("publicResourcesLabel");
    expect(inspectorSource).not.toContain("props.faq.slice");
    expect(inspectorSource).not.toContain('id="profile-knowledge"');
    expect(inspectorSource.match(/<details className="representative-inspector-section"/g)?.length).toBeGreaterThanOrEqual(2);
    expect(inspectorSource).not.toContain("props.capabilities");
    expect(inspectorSource).not.toContain("capabilitiesLabel");
    ...
    expect(inspectorSource).toContain("document.body");
    ...
    expect(inspectorSource).not.toContain('openModal("privacy"');
    expect(inspectorSource).not.toContain("AI 先接待，需要判断时可转真人。");
    expect(inspectorSource).not.toContain("查看完整说明");
  });
  ...
  it("passes only public profile, catalog, resource, and trust projections", () => {
    ...
    expect(pageSource).not.toContain("capabilities={representative.skills");
