...
export async function requestOwnerProfile(principal: IdentityProfilePrincipal, action: 'get' | 'avatar' | 'avatar-upload', avatar?: unknown): Promise<OwnerIdentityProfile> {
  ...
  const response = await fetch(new URL('/internal/owner-profile', endpoint), {
    method: 'POST', redirect: 'error', cache: 'no-store', signal: AbortSignal.timeout(action === "avatar-upload" ? 60_000 : 15_000),
    headers: { authorization: `Bearer ${token}`, 'content-type': 'application/json' },
    body: JSON.stringify({ principal, action, ...(action !== 'get' ? { avatar } : {}) }),
  });
  if (!response.ok) throw new IdentityProfileError([400, 401, 403, 413, 415, 502, 503].includes(response.status) ? response.status : 503, 'Account profile request failed.');
