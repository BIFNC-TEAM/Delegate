...
export function DashboardAccountProfile({ locale, available = true, children, mode = "profile", socialHref }: { locale: Locale; available?: boolean; children?: ReactNode; mode?: "profile" | "connections"; socialHref?: string }) {
  const zh = locale === 'zh';
  ...
  if (mode === 'connections') return <div className="account-profile-content">
    {error && <p role="alert" className="settings-field-error">{error} <button type="button" className="dashboard-v2-button-secondary" onClick={() => { void load(); }}>{zh ? '重试' : 'Retry'}</button></p>}
    {!profile && !error && <p role="status">{zh ? '正在读取三方账号…' : 'Loading linked accounts…'}</p>}
    {profile && accounts.length === 0 && <p className="settings-action-note">{zh ? '当前没有可用的三方账号服务。' : 'No third-party account providers are available.'}</p>}
    {accounts.map((account) => <div className="account-profile-method" key={account.provider}>
      <div><strong>{providerName(account)}</strong><p>{account.linked ? (zh ? '已绑定' : 'Linked') : (zh ? '尚未绑定' : 'Not linked')}</p></div>
  ...
  return <div className="account-profile-content">
    ...
    {profile && <>
      ...
      <div className="account-profile-method"><div><strong>{zh ? '三方账号绑定' : 'Third-party accounts'}</strong>
        {accounts.length ? accounts.map((account) => <p key={account.provider}>{providerName(account)} · {account.linked ? (zh ? '已绑定' : 'Linked') : (zh ? '尚未绑定' : 'Not linked')}</p>) : <p>{zh ? '暂无可用服务' : 'No providers available'}</p>}
      </div>{socialHref && <a className="dashboard-v2-button-secondary" href={socialHref}>{zh ? '管理三方账号' : 'Manage linked accounts'}</a>}</div>
    </>}
  </div>;
}
