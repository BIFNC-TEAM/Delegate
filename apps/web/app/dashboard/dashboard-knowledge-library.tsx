...
const visibilityOptions: Array<{ value: Visibility; zh: string; en: string; detailZh: string; detailEn: string }> = [
  ...
  { value: "selected_representatives", zh: "指定代表", en: "Selected representatives", detailZh: "仅关联的数字代表可使用", detailEn: "Only linked representatives can use it" },
    ...
    <>
      ...
      <header className="knowledge-page-header">
        <div>
          ...
          <span>{zh ? "统一管理文件、网页和手工知识，并明确哪些数字代表可以在什么场景下使用。" : "Manage files, webpages, and authored knowledge with explicit representative permissions."}</span>
        </div>
      ...
      {importOpen ? <KnowledgeImportDialog activeSlug={activeSlug} locale={locale} onClose={() => setImportOpen(false)} onCreated={(asset) => { setAssets((current) => [asset, ...current.filter((item) => item.id !== asset.id)]); setSelectedId(asset.id); setImportOpen(false); setNotice(zh ? "知识已导入并完成处理。" : "Knowledge imported and processed."); }} representatives={representatives} /> : null}
...
function KnowledgeImportDialog({ activeSlug, locale, representatives, onClose, onCreated }: { activeSlug: string; locale: Locale; representatives: KnowledgeRepresentativeOption[]; onClose: () => void; onCreated: (asset: KnowledgeAssetRecord) => void }) {
  useModalBehavior(onClose, ".knowledge-import-dialog");
  const zh = locale === "zh";
  ...
  async function submit(event: FormEvent) {
    ...
    try {
      ...
      if (mode === "file") {
        if (!file) throw new Error(zh ? "请选择知识文件。" : "Choose a knowledge file.");
        ...
        response = await knowledgeFetch(activeSlug, "/api/dashboard/knowledge-assets", { method: "POST", body: form });
      } else {
        response = await knowledgeFetch(activeSlug, "/api/dashboard/knowledge-assets", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            kind: mode,
  ...
  return <div className="knowledge-modal-backdrop" onMouseDown={(event) => { if (event.currentTarget === event.target) onClose(); }} role="presentation">
    <section aria-label={zh ? "导入知识" : "Import knowledge"} aria-modal="true" className="knowledge-import-dialog" role="dialog">
      <header><div><span>Knowledge Intake</span><h2>{zh ? "导入新的知识" : "Import new knowledge"}</h2><p>{zh ? "导入后会自动提取正文、生成摘要与标签，并按权限连接代表。" : "Delegate extracts content, creates a summary and tags, then applies representative access."}</p></div><button aria-label={zh ? "关闭" : "Close"} onClick={onClose} type="button">×</button></header>
      <nav className="knowledge-import-tabs">{(["file", "url", "text"] as ImportMode[]).map((item) => <button className={mode === item ? "is-active" : undefined} key={item} onClick={() => setMode(item)} type="button"><span>{item === "file" ? "▤" : item === "url" ? "↗" : "Aa"}</span>{item === "file" ? (zh ? "上传文件" : "Upload file") : item === "url" ? (zh ? "导入网址" : "Import URL") : (zh ? "手工文本" : "Manual text")}</button>)}</nav>
      <form onSubmit={submit}>
        {mode === "file" ? <div className={`knowledge-dropzone${file ? " has-file" : ""}`} onClick={() => fileRef.current?.click()} onDragOver={(event) => event.preventDefault()} onDrop={(event) => { event.preventDefault(); setFile(event.dataTransfer.files[0] ?? null); }}><input accept=".pdf,.docx,.txt,.md,.markdown" onChange={(event) => setFile(event.target.files?.[0] ?? null)} ref={fileRef} type="file" /><span>{file ? fileMark(file.name.split(".").pop()?.toLowerCase() as KnowledgeAssetRecord["kind"] ?? "txt") : "↑"}</span><strong>{file?.name ?? (zh ? "拖入文件，或点击选择" : "Drop a file, or click to browse")}</strong><small>{file ? formatBytes(file.size) : (zh ? "PDF、DOCX、TXT、Markdown · 最大 15 MB" : "PDF, DOCX, TXT, Markdown · 15 MB max")}</small></div> : mode === "url" ? <label className="knowledge-form-field"><span>{zh ? "公开网址" : "Public URL"}<b>*</b></span><input onChange={(event) => setSourceUrl(event.target.value)} placeholder="https://example.com/about" required type="url" value={sourceUrl} /><small>{zh ? "只支持可公开访问的 HTTP/HTTPS 页面，不会访问内网地址。" : "Public HTTP/HTTPS pages only. Private network addresses are blocked."}</small></label> : <label className="knowledge-form-field"><span>{zh ? "知识正文" : "Knowledge text"}<b>*</b></span><textarea minLength={20} onChange={(event) => setSourceText(event.target.value)} placeholder={zh ? "粘贴 FAQ、服务说明、政策、品牌信息或其他可复用知识…" : "Paste FAQs, service guidance, policies, brand context, or other reusable knowledge…"} required rows={8} value={sourceText} /><small>{sourceText.length.toLocaleString()} / 400,000</small></label>}
        <div className="knowledge-form-grid"><label className="knowledge-form-field"><span>{zh ? "知识标题" : "Title"}<b>*</b></span><input maxLength={180} onChange={(event) => setTitle(event.target.value)} placeholder={mode === "file" ? (zh ? "不填则使用文件名" : "Defaults to filename") : (zh ? "例如：产品与服务 FAQ" : "e.g. Product and service FAQ")} required={mode !== "file"} value={title} /></label><label className="knowledge-form-field"><span>{zh ? "手工标签" : "Tags"}</span><input onChange={(event) => setTags(event.target.value)} placeholder={zh ? "产品, FAQ, 价格" : "Product, FAQ, pricing"} value={tags} /><small>{zh ? "用逗号分隔，最多 20 个" : "Comma separated, up to 20"}</small></label></div>
        ...
        {(visibility === "selected_representatives" || visibility === "public_material") ? <fieldset className="knowledge-representative-picker"><legend>{zh ? "关联数字代表" : "Linked representatives"}{visibility === "selected_representatives" ? <b>*</b> : null}</legend>{representatives.map((representative) => <label key={representative.id}><input checked={linkedIds.includes(representative.id)} onChange={(event) => setLinkedIds((current) => event.target.checked ? [...current, representative.id] : current.filter((id) => id !== representative.id))} type="checkbox" /><span>{representative.name.slice(0, 1)}</span><div><strong>{representative.name}</strong><small>/{representative.slug}</small></div></label>)}{!representatives.length ? <p>{zh ? "当前工作区还没有可关联的代表。" : "No representatives are available in this workspace."}</p> : null}</fieldset> : null}
        {error ? <div className="knowledge-form-error">! {error}</div> : null}
        <footer><button onClick={onClose} type="button">{zh ? "取消" : "Cancel"}</button><button className="is-primary" disabled={submitting || (visibility === "selected_representatives" && !linkedIds.length)} type="submit">{submitting ? <><i className="knowledge-spinner" />{zh ? "正在处理…" : "Processing…"}</> : <>{zh ? "开始处理" : "Start processing"}<span>→</span></>}</button></footer>
      </form>
    </section>
  </div>;
}
