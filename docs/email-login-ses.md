...
- Logto 1.41 的免密码注册模式要求非 username 登录方式同时启用验证码，所以没有邮件连接器时不能安全地仅打开 email/password。当前本地已关闭用户名登录，已配置 SES 并启用 phone/email 方法及 Account Center email Edit。
...
要求发信域名已验证、发信地址可用、验证码模板审核通过且包含 `{{code}}` 和 `{{expireMinutes}}`；验证码变量名不同时用 CODE_VARIABLE 指定（不得与 expireMinutes 重名）。有效期由配置工具读取当前 Logto 策略的秒数并除以 60，支持更短策略及非整数分钟；未知策略拒绝配置。统一登录策略工具会阻止模板有效期不一致的切换；手动更改 Logto 验证码策略后必须重新运行 SES 配置工具同步。地址、模板、地域必须匹配。密钥应具备 SES SendEmail 权限。密钥不得写进前端、日志或 Git。
...
SES 配置工具会备份原连接器到私有文件并回读校验；发现其他邮件供应商时拒绝静默覆盖。真实环境不要使用 `--mock-sms`，且需单独部署本分支的连接器和应用代码。本次未修改线上登录策略。本地已使用香港地域、发信地址 `noreply@mail.rag8.cn`、模板 `219548`；仅向用户指定邮箱发送了一封原生 Generic 验证测试邮件，Logto 返回 204（SES 接受），尚待确认实际收件和模板渲染。这封测试邮件不会绑定邮箱，也不能替代设置页发起的绑定验证码。
...
| 真实邮件投递与绑定 | 实际 SES 密钥、发信地址和审核模板 | 收到邮件后完成原生邮箱验证 | 本地原生验证码请求返回 204，SES 已接受；实际收件及真实账号绑定尚待验证 |
...
## 2026-09-22 香港 SES 联调补充

- 配置回读：地域 ap-hongkong；phone/email 登录；邮箱绑定字段 Edit；Logto 有效期 300 秒；邮件变量 expireMinutes 为 5。
- 模板现在携带实际 code 和 expireMinutes，不将本地短信 mock 的固定码用于邮件。
- 本次补充测试覆盖较短的 90 秒策略、缺失策略、无效有效期、变量名冲突、统一登录策略与模板有效期不一致时拒绝写入。
- 本次命令：`pnpm test:logto:unified`（41 项）、`pnpm test:logto:ses:runtime`（3 项）。前文其余完整回归结果属于上一次实现验证，本次未重新运行完整工作区测试。

本次修改文件及用途：

| 文件 | 本次修改 |
| --- | --- |
| `deploy/logto/connectors/connector-tencent-ses/lib/ses.js` | 向 SES 传入 expireMinutes，校验有效期及变量名冲突 |
| `deploy/logto/connectors/connector-tencent-ses/lib/index.js` | 声明连接器有效期配置及表单字段 |
| `scripts/logto-ses-auth.mjs` | 从实际 Logto 策略读取有效期，缺失时拒绝配置 |
| `scripts/logto-unified-auth.mjs` | 拒绝模板与目标登录策略有效期不一致的变更 |
| `scripts/tests/logto-ses-auth.test.ts` | 更新独立签名参考，覆盖双变量、短有效期、缺失策略与非法配置 |
| `scripts/tests/logto-unified-auth.test.ts` | 验证有效期不一致时不写入任何配置 |
| `scripts/tests/logto-ses-auth.runtime.mjs` | 在固定版本连接器运行时验证双变量负载 |
| `deploy/logto/tencent-ses.env.example` | 示例使用香港地域，解释模板变量 |
| `deploy/logto/connectors/connector-tencent-ses/README.md` | 说明有效期取值及策略变更后的同步步骤 |
| `docs/unified-auth-runbook.md` | 更新当前本地 SES 启用及待验收状态 |
| `docs/email-login-ses.md` | 记录此次配置、测试和验收边界 |

私有 `.local/logto/tencent-ses.env` 的地域配置已更新，不纳入提交；密钥未进入版本控制。既有未跟踪目录 `reports/agent-timing/` 保留原状。
