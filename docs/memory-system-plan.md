...
| T6 | 已完成 | UseRun/UseItem 真相账本区分搜索、校验、模型注入、回复引用和公开展示 |
| T7 | 已完成 | 真实业务 API、四个 URL 深链页面及代表级 Memory 设置已落地 |
| T8 | Web P0 已完成 | 公开端发送前披露、12×12 隔离门禁和回归门禁已落地；Matrix/Telegram 在具备等价披露前保持禁用 |
...
P0 只开放 Web 的长期记忆召回与提取。Matrix/Telegram 当前没有可证明在首次发送前完成等价记忆披露的产品流程，因此 API、设置和运行时统一返回不支持并 fail closed；完成渠道披露、删除入口和回归验证后才能分别开启。旧 `/training/*` 与旧 OpenViking 管理路由保留无 UI 调用的兼容期，按 T8 的“稳定一个发布周期”条件再删除。
...
通过完整隔离、安全、草稿、纠正、删除、同步、对账、权限和公开披露测试后，按 Shadow -> Web 内部 -> Web 单渠道小流量灰度。Matrix/Telegram 必须分别完成首次发送前披露和同等门禁后，才能进入自己的灰度。稳定一个发布周期且确认无调用后，归档/导出旧数据并删除旧 `/training/*`、Creator Training 服务、测试和 Prisma 模型。
