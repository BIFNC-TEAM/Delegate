-- Execute retention lifecycle and permit only reversible policy withdrawals to
-- be re-projected. Explicit deletion remains irreversible because its proof is
-- append-only and is checked at this database transition boundary.
...
DECLARE
  ...
  policy_reenable_requeue BOOLEAN := FALSE;
BEGIN
  ...
  IF TG_OP = 'UPDATE'
     AND OLD."status" IN ('DELETE_PENDING', 'DELETE_FAILED', 'DELETED')
     AND NEW."status" = 'QUEUED'::"MemoryProjectionStatus" THEN
    ...
    policy_reenable_requeue :=
      OLD."lane" = 'RECALL'::"MemoryProjectionLane"
      AND OLD."deleteRequestedAt" IS NOT NULL
      AND NEW."lane" = OLD."lane"
      AND NEW."provider" = OLD."provider"
      AND NEW."memoryId" = OLD."memoryId"
      AND NEW."memoryVersionId" = OLD."memoryVersionId"
      AND NEW."contentHash" = OLD."contentHash"
      AND NEW."remoteObjectId" IS NULL
      AND NEW."writeReceiptHash" IS NULL
      AND NEW."writeVerifiedAt" IS NULL
      AND NEW."deleteReceiptHash" IS NULL
      AND NEW."remoteAbsentAt" IS NULL
      AND NEW."attemptCount" = 0
      AND NEW."leaseToken" IS NULL
      AND NEW."leaseExpiresAt" IS NULL
      AND NEW."projectedAt" IS NULL
      AND NEW."deleteRequestedAt" IS NULL
      ...
      AND version_record."contentHash" = NEW."contentHash"
      ...
      AND memory_record."currentVersionId" = NEW."memoryVersionId"
      ...
      AND policy_record."provider" = NEW."provider"
    ...
    IF NOT policy_reenable_requeue THEN
      RAISE EXCEPTION USING
        ERRCODE = '23514',
        CONSTRAINT = 'MemoryProjectionItem_policy_reenable_requeue_check',
        MESSAGE = 'only a current safe policy-withdrawn projection may be requeued';
    END IF;
  END IF;
...
-- Receipt and attempt evidence is normally append-only. A policy withdrawal is
-- the sole exception: after exact deletion (or before its claim), a proof-free
-- projection may start a fresh execution chain when the current memory and
-- current policy make it recall-eligible again.
