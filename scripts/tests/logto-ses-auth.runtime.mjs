...
const config={secretId:'synthetic-id',secretKey:'synthetic-key',region:'ap-guangzhou',fromEmail:'noreply@example.com',templateId:1001,codeVariable:'code',expireMinutes:5,subject:'Delegate verification'};
...
test('native connector forwards email binding OTP to SES and propagates failures',async()=>{
 ...
 try{
  ...
  assert.equal(sent.url,'https://ses.tencentcloudapi.com/');assert.equal(sent.body.Template.TemplateData,'{"code":"123456","expireMinutes":5}');
