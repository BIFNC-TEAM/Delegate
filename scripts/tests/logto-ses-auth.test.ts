...
const config={secretId:'AKIDEXAMPLE',secretKey:'SECRETEXAMPLE',region:'ap-guangzhou',fromEmail:'noreply@example.com',templateId:1001,codeVariable:'code',subject:'Delegate verification'};
const message={to:'person@example.com',type:'BindNewIdentifier',payload:{code:'123456'}};
describe('Tencent SES email verification',()=>{
 it('matches an independently computed TC3 signature and sends only the verification code to the approved template',()=>{
  const request=createSesRequest(message,config,1700000000);
  expect(request.headers.Authorization).toBe('TC3-HMAC-SHA256 Credential=AKIDEXAMPLE/2023-11-14/ses/tc3_request, SignedHeaders=content-type;host, Signature=e58bb50b6564aafbd9e8850e11c02663073c7519cf46bb217b4fa2d5f13f5087');
  expect(JSON.parse(request.body)).toEqual({FromEmailAddress:config.fromEmail,Destination:[message.to],Subject:config.subject,Template:{TemplateID:1001,TemplateData:'{"code":"123456"}'}});
 });
 ...
 it('backs up and verifies configured SES without exposing credentials in its result',async()=>{
  ...
  const backup=vi.fn();const savedConfig=readSesConfig(env);
 ...
 it('refuses to overwrite an unrelated configured email provider',async()=>{
  const request=vi.fn(async(path:string,method?:string)=>path==='/api/connectors'?[{type:'Email',connectorId:'other',id:'existing'}]:path==='/api/connector-factories'?[]:{issuer:'http://127.0.0.1:3301/oidc'});
