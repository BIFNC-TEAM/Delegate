...
FIXTURE_CONTAINER="delegate-service-entitlements-pg16-$$"
...
command -v docker >/dev/null 2>&1 || {
  printf 'Docker is required for the disposable PostgreSQL 16 entitlement fixture.\n' >&2
  exit 2
}
command -v pnpm >/dev/null 2>&1 || {
  printf 'pnpm is required for the PostgreSQL 16 entitlement fixture.\n' >&2
  exit 2
}
...
docker run \
  ...
  --env POSTGRES_PASSWORD=service_entitlement_fixture_only \
  --env POSTGRES_DB=delegate_entitlement_test \
  --publish 127.0.0.1::5432 \
  --health-cmd='pg_isready -U postgres -d delegate_entitlement_test' \
  --health-interval=1s \
  --health-timeout=2s \
  --health-retries=30 \
...
FIXTURE_DATABASE_URL="postgresql://postgres:service_entitlement_fixture_only@127.0.0.1:${FIXTURE_PORT}/delegate_entitlement_test"
